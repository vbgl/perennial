...
Section refinement_triples.
  ...
  Definition ExecInv :=
    (source_ctx ∗ ∃ γlock, is_staged_lock lN lN' γlock lock_addr ExecLockInv ExecLockInv)%I.
  ...
  Proof.
    ...
    { set_solver+. }
    rewrite /ExecLockInv.
    iDestruct "HEL" as (ptr_val pcurr pother) ">(Hsource&Hread&H1&H2&H3&H4)".
    wp_step.
    iMod("Hclose" with "[Hsource Hread H1 H2 H3 H4]") as "HEL".
    { iSplitR ""; last first.

      iAlways. iIntros. iExists _, _, _. eauto.
      iFrame.
    }
    iModIntro.
    ...
    iMod (staged_inv_open with "[$]") as "(HEL&Hclose)".
    { set_solver+. }
    (* We now already remember exactly what pcurr pother are,
       as we should by force of holding the lock, instead of having to
       unify the lease with the master *)
    iDestruct "HEL" as ">(Hsource&Hread&H1&H2&H3&H4)".
    wp_step.
    iMod("Hclose" with "[Hsource Hread H1 H2 H3 H4]") as "HEL".
    { iSplitR ""; last first.
      iAlways. iIntros. iExists _, pcurr, (_, _). eauto.
      iFrame.
    }
    ...
    iMod("Hclose" with "[Hsource Hread H1 H2 H3 H4]") as "HEL".
    { iSplitR ""; last first.
      iAlways. iIntros. iExists (swap_ptr ptr_val), (new_fst, new_snd), (_, _). eauto.
      iFrame. simpl. rewrite ?read_of_swap ?write_of_swap; iFrame.
    }

    iModIntro.
    iApply (wp_staged_unlock with "[Hlockinv HEL Hlocked]").
    { iFrame "Hlockinv". iFrame. (* we should alloow weakening ? *)
