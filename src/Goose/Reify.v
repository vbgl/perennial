...
Ltac refl' RetB RetT e :=
  ...
  | fun x : ?T => (match ?r in NonAtomicArgs _ return ?T1 with (FinishArgs _) => (@?s1 x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit s1 in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r return T1 with (FinishArgs _) => (f1 x) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return ?T1 with (FinishArgs fa) => (@?s1 fa x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit (fun (p: _ * T) => (s1 (fst p) (snd p))) in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r return T1 with (FinishArgs fa) => (f1 (fa, x)) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return _ with (FinishArgs _) => (@?s1 x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit s1 in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r with (FinishArgs _) => (f1 x) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return _ with (FinishArgs fa) => (@?s1 fa x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit (fun (p: _ * T) => (s1 (fst p) (snd p))) in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r with (FinishArgs fa) => (f1 (fa, x)) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return _ with (FinishArgs _) => (@?s1 x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit s1 in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r return _ with (FinishArgs _) => (f1 x) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return _ with (FinishArgs fa) => (@?s1 fa x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit (fun (p: _ * T) => (s1 (fst p) (snd p))) in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r return _ with (FinishArgs fa) => (f1 (fa, x)) | Begin => (f2 x) end)

  (* This matches for the data Ops *)
  | fun x : ?T => (match ?r with (FinishArgs _) => (@?s1 x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit s1 in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r with (FinishArgs _) => (f1 x) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r with (FinishArgs fa) => (@?s1 fa x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit (fun (p: _ * T) => (s1 (fst p) (snd p))) in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r with (FinishArgs fa) => (f1 (fa, x)) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return (relation fs fs (retT ?r (slice.t string))) with (FinishArgs _) => (@?s1 x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit s1 in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r with (FinishArgs _) => (f1 x) | Begin => (f2 x) end)

  | fun x : ?T => (match ?r in NonAtomicArgs _ return (relation fs fs (retT ?r (slice.t string))) with (FinishArgs fa) => (@?s1 fa x) | Begin => (@?s2 x) end) =>
    let f1 := refl' fs unit (fun (p: _ * T) => (s1 (fst p) (snd p))) in
    let f2 := refl' fs unit s2 in
    constr: (fun x => match r with (FinishArgs fa) => (f1 (fa, x)) | Begin => (f2 x) end)
